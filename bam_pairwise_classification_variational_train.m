...
function state = bam_pairwise_classification_variational_train(X, y, parameters)
    ...
    while 1
        ...
        Zeta(positive, negative) = repmat(mu_exp_f(positive), 1, length(negative)) + repmat(mu_exp_f(negative)', length(positive), 1);
        %%%% update f.mu
        mu_f_star = minFunc(@L_q_mu_f, f.mu, options, bw.mu, X, f.sigma, Zeta, negative, positive);
        f.mu = mu_f_star;
        %%%% update Zeta
        mu_exp_f = exp(f.mu + f.sigma / 2);
        Zeta(positive, negative) = repmat(mu_exp_f(positive), 1, length(negative)) + repmat(mu_exp_f(negative)', length(positive), 1);
        %%%% update f.sigma
        log_sigma_f_star = minFunc(@L_q_sigma_f, log(f.sigma), options, bw.mu, X, f.mu, Zeta, negative, positive);
        ...
        %%%% p(z | f)
        mu_exp_f = exp(f.mu + f.sigma / 2);
        lb = lb + sum(f.mu(positive)) * length(negative) - sum(sum((repmat(mu_exp_f(positive), 1, length(negative)) + repmat(mu_exp_f(negative)', length(positive), 1)) ./ Zeta(positive, negative))) + length(positive) * length(negative) - sum(sum(log(Zeta(positive, negative))));
...
function [L_q, d_L_q] = L_q_mu_f(mu_f, bw, X, sigma_f, Zeta, negative, positive)
    N = size(X, 2);
    mu_f_est = [ones(1, N); X]' * bw;
    mu_exp_f = exp(mu_f + sigma_f / 2);
    %lower bound
    L_q = sum((mu_f.^2 + sigma_f - 2 * mu_f .* mu_f_est) / 2) - sum(mu_f(positive)) * length(negative) + sum(sum((repmat(mu_exp_f(positive), 1, length(negative)) + repmat(mu_exp_f(negative)', length(positive), 1)) ./ Zeta(positive, negative))) + sum(sum(log(Zeta(positive, negative)))) - sum(log(2 * pi * sigma_f) / 2);
    %first derivative
    d_L_q = (mu_f - mu_f_est);
    d_L_q(positive) = d_L_q(positive) - length(negative) + sum(repmat(mu_exp_f(positive), 1, length(negative)) ./ Zeta(positive, negative), 2);
    d_L_q(negative) = d_L_q(negative) + sum(repmat(mu_exp_f(negative)', length(positive), 1) ./ Zeta(positive, negative), 1)';
end

function [L_q, d_L_q] = L_q_sigma_f(log_sigma_f, bw, X, mu_f, Zeta, negative, positive)
    ...
    L_q = sum((mu_f.^2 + sigma_f - 2 * mu_f .* mu_f_est) / 2) - sum(mu_f(positive)) * length(negative) + sum(sum((repmat(mu_exp_f(positive), 1, length(negative)) + repmat(mu_exp_f(negative)', length(positive), 1)) ./ Zeta(positive, negative))) + sum(sum(log(Zeta(positive, negative)))) - sum(log(2 * pi * sigma_f) / 2);
    %first derivative
    d_L_q = 1 / 2 - 1 ./ (2 * sigma_f);
    d_L_q(positive) = d_L_q(positive) + sum(repmat(mu_exp_f(positive), 1, length(negative)) ./ (2 * Zeta(positive, negative)), 2);
    d_L_q(negative) = d_L_q(negative) + sum(repmat(mu_exp_f(negative)', length(positive), 1) ./ (2 * Zeta(positive, negative)), 1)';
end
